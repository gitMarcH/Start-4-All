...
This Statistical Analysis Plan (SAP) v1.0 is based on the Start 4 All International Protocol v1.6, dated 22 May 2023.
...
### Threshold for diagnostic tests: CRP and CXR-CAD
...
For notation, to denote that we will use `-->` to refer to one assay followed by another assay and `|` to refer to assays being administered in parallel. In other words, combination `A | B --> C` refers to a an algorithm where if either assay A or assay B gives a positive result (or both are positive) then proceed to assay C (and record the outcome from that assay), else record a negative result.

#### PCF/ICF

We will distinguish between lowest level of care and district-level settings, but the algorithms evaluated are the same.

1. `Xpert Ultra with pooling`
2. `Xpert Ultra`
3. `LAM`
4. `CXR-CAD --> Xpert Ultra with pooling`
5. `CRP --> Xpert Ultra with pooling`
6. `LAM --> Xpert Ultra with pooling`
7. `CXR-CAD | CRP | LAM --> Xpert Ultra with pooling`
8. `CXR-CAD | LAM --> Xpert Ultra with pooling`
9. `CRP | LAM --> Xpert Ultra with pooling`
10. `CXR-CAD | CRP --> Xpert Ultra with pooling`
...
15. `CXR-CAD | LAM --> Xpert Ultra`
16. `CRP | LAM --> Xpert Ultra`
17. `CXR-CAD | CRP --> Xpert Ultra with pooling`
18. `CXR-CAD --> LAM`
19. `CRP --> LAM`
20. `CXR-CAD | CRP --> LAM`
...
2. `Xpert Ultra`
3. `LAM`
4. `CRP --> Xpert Ultra with pooling`
5. `LAM --> Xpert Ultra with pooling`
6. `CRP | LAM --> Xpert Ultra with pooling`
7. `CRP --> Xpert Ultra`
8. `LAM --> Xpert Ultra`
...
with $\mbox{logit}(p_{TB})=\beta_0+\beta_{s}\cdot{sex}+\beta_{h}\cdot{hiv}+\beta_a\cdot\mbox{ageGroup}$, assuming weakly informative $\cal{N}(0,10)$ priors for the $\beta_j$ parameters. For the analyses stratified by HIV status, the HIV term is dropped from the linear predictor term.
...
with $\mbox{logit}(p_{TB,l})=\beta_0+\beta_{s}\cdot{sex}+\beta_{h}\cdot{hiv}+\beta_a\cdot\mbox{ageGroup}+\beta_l\cdot country$, assuming weakly informative $\cal{N}(0,10)$ priors for the $\beta_j$ parameters. For the analyses stratified by HIV status, the HIV term is dropped from the linear predictor term.
...
This version of the SAP does not include mock-ups as they need to be re-done completely given the change in algorithms being evaluated. While the analysis plan itself is complete, mock-up will only be included in the next version.
