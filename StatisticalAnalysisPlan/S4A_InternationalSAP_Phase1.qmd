...
This Statistical Analysis Plan (SAP) v1.5 is based on the Start 4 All International Protocol v2.0, dated 12 June 2024.
...
dfHist<-data.frame(
  Version=c("1.5","1.4","1.3","1.2","1.1","1.0"),
  Date=c("2025-05-28","2025-05-27","2025-05-27","2024-05-27","2025-05-26","2025-05-25"),
  Author=c("Marc Henrion","Marc Henrion","Marc Henrion","Marc Henrion","Marc Henrion","Marc Henrion"),
  Edits=c("Added secondary analysis treating pooled Xpert --> Xpert as a single step and assuming conditional independence; Typo correction","Slight changes to protocol summary text; revised language for PCF/ICF (facility-based) ACF (community-based) and pooled (aggregate) analysis; addition of mock-ups for community-based CXR-CAD entry to study","All analyses tested; mock-up output for PCF/ICF setting included","Clarified agorithms with pooled Xpert Ultra testing and updated JAGS files correspondingly","Descriptive analysis mock-ups added; full JAGS model files added; details of MCMC run parameters; added JAGS version number; slightly revised secondary analyses","First full version of the SAP (while development versions 0.1 to 0.14 exist, no version history record was kept prior to v1.0)")
)
...
dfCountries<-data.frame(
  Population=c("Primary health care attenders","District / secondary hospital attenders","Key / marginalised populations (informal settlements, IPDs, rural poor, pastoralists)","Children"),
...
##### Community-based - CXR-CAD entry or (CXR-CAD or W4SS) entry
...
#### Aggregate facility-based analysis at lowest level of care
...
* Treating pooled molecular testing (the step `Pooled Xpert Ultra --> Xpert Ultra` from @sec-algos) as a single test. With this change, the conditional independence assumption of tests given TB status is more reasonable, and so we will also derive all non-single test performance parameters directly from the estimated TB prevalence and performance metrics (primarily sensitivity and specificity) of the individual tests (CRP, CXR-CAD, LAM, pooled molecular testing). This means that in Step 4 of the estimation process (for the per-country as well as the aggregate analysis) in @sec-estimation we assume a diagonal covariance matrix, and steps 6 and 7 are redundant.
...
Below in @tbl-primAnalPcfIcf we present how we plan to present results for the overall facility-based evaluation at lowest levels of primary care.

```{r}
#| label: tbl-primAnalPcfIcf
#| tbl-cap: "Facility-based evaluation (all participants): Bayesian posterior estimates for sensitivity, specificity, positive and negative predictive values and positive and negative likelihood ratios for PHC / lowest level of primary care settings."
...
In figure @fig-forestPcfIcf_PHC we show how we plan to summarise sensitivities and specificities (the primary interest of the performance evaluation).

```{r}
#| label: fig-forestPcfIcf_PHC
#| fig-cap: "Facility-based evaluation (all participants): forest plots showing the Bayesian posterior estimates for sensitivity and specificity for PHC / lowest level of primary care settings."
#| fig-width: 11
#| fig-height: 40
...
These will be presented similarly, though involve a different set of evaluated algorithms (see @sec-algos).

```{r}
#| label: tbl-primAnalAcfCxr
#| tbl-cap: "Community-based evaluation (all participants; CXR entry to study): Bayesian posterior estimates for sensitivity, specificity, positive and negative predictive values and positive and negative likelihood ratios."
...
In figure @fig-forestAcfCxr we show how we plan to summarise sensitivities and specificities (the primary interest of the performance evaluation).

```{r}
#| label: fig-forestAcfCxr
#| fig-cap: "Community-based evaluation (all participants; CXR-CAD entry to study): forest plots showing the Bayesian posterior estimates for sensitivity and specificity for PHC / lowest level of primary care settings."
#| fig-width: 11
#| fig-height: 14
...
dfResPrimBetaBayes<-data.frame(
  country=c(rep("Cameroon",4),rep("Nigeria",4),rep("Kenya",2),rep("Bangladesh",3),rep("Brazil",2),rep("Viet Nam",3),rep("Malawi",1)),
...
@fig-forestPcfIcf_PHC: Facility-based evaluation (all participants): forest plots showing the Bayesian posterior estimates for sensitivity and specificity for PHC / lowest level of primary care settings.

@fig-forestAcfCxr: Community-based evaluation (all participants; CXR-CAD entry to study): forest plots showing the Bayesian posterior estimates for sensitivity and specificity for PHC / lowest level of primary care settings.
...
@tbl-primAnalPcfIcf: Facility-based evaluation (all participants): Bayesian posterior estimates for sensitivity, specificity, positive and negative predictive values and positive and negative likelihood ratios for PHC / lowest level of primary care settings.

@tbl-primAnalAcfCxr: Community-based evaluation (all participants; CXR entry to study): Bayesian posterior estimates for sensitivity, specificity, positive and negative predictive values and positive and negative likelihood ratios.
