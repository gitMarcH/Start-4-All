...
This Statistical Analysis Plan (SAP) v0.4 is based on the Start 4 All International Protocol v1.6, dated 22 May 2023.
...
@tbl-diagnostics summarises the diagnostic screening assays evaluated in this study.

```{r diagnostics}
#| echo: false
#| label: tbl-diagnostics
#| tbl-cap: "Summary of diagnostic screening assays evaluated in this study."
...
analysisFun<-function(dat,c,r,s){
  ...
  specHiv=ifelse(sum(dat$reference==1)>0,propEstCi(k=sum(dat$reference==0 & dat$hiv==0),n=sum(dat$reference==0)),NA)
...
dfResPrimBeta %>%
  dplyr::select(!country) %>%
  kable(row.names=FALSE,col.names=c("Region","Setting",rep(c("a","b"),31))) %>%
  kable_styling(full_width=FALSE) %>%
...
For all of the above parameters, we will estimate them unconditionally (i.e. for each test separately) and conditionally on any combination of prior test results from any number of the other tests (maximum of 4 prior tests given 6 tests in total).
