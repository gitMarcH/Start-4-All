...
In our case, the prior distribution we will use a beta prior distribution, $p\sim\beta(a,b)$, where $p$ is the probability parameter of interest, and a binomial likelihood $X\sim Bin(n,p)$, where $X$ is the number of positive results, $n$ the total number of observations and $p$ the same probability parameter as in the prior. One can show that the posterior will also be a beta distribution (since the beta is the conjugate prior for a binomial likelihood) with a closed form solution: $p|X=k\sim\beta(k+a,n-k+b)$. The closed-form solution means this is computationally extremely fast to compute. For prior, a weakly informative prior will be used, specifically the Kerman prior $\beta(1/3,1/3)$ [@kerman2011].
