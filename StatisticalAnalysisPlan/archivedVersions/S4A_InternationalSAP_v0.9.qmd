...
* Add LR+ and LR- as performance metrics for individual tests and overall assessment of combinations. Note that since we use closed form solutions for the Bayesian model fitting, for LR+ and LR-, we will have to estimate these in the frequentist way since we cannot simply sample from the posteriors for sensitivity and specificity as these are not independent parameters... We could use MCMC (and then simply derive sets of values for LR+ and LR- from the pairs of sensitivity and specificity estimates) but computationally too demanding given the number of tests and test combinations we need to evaluate. (See [https://www2.ccrb.cuhk.edu.hk/stat/confidence%20interval/Diagnostic%20Statistic.htm](https://www2.ccrb.cuhk.edu.hk/stat/confidence%20interval/Diagnostic%20Statistic.htm) and [https://doi.org/10.1136%2Fbmj.329.7458.168](https://doi.org/10.1136%2Fbmj.329.7458.168) for deriving CIs; basically treating LR+, LR- as risk ratios.)

    + LR+ = sensitivity / 1- specificity
    + LR- = 1- sensitivity / specificity

* Remove the Jan15 queries and update as per discussions from the meeting.

* Start implementing some of the diagnostic combinations as shared by Tushar (email from 31 January 2024 + later updates).

* Agree on parameter names with Amanda - tables to have all possible parameters...
...
This Statistical Analysis Plan (SAP) v0.9 is based on the Start 4 All International Protocol v1.6, dated 22 May 2023.
...
For the primary objective we will estimate performance metrics for individual screening tests as well as combinations ("screening algorithms") of tests. Specifically, we will estimate:
...
In Bayesian estimation, we estimate the posterior probability density of a given parameter; *posterior* because it is the best-fit distribution after we have observed data. The posterior distribution summarises our beliefs about the likely values for the parameter of interest and it is an update of our beliefs about the parameter before we had observed data -- the prior distribution. The data are generated by a stochastic process and we can compute the likelihood of the data under a distribution for this process. To summarise:

$$\mbox{posterior} = \mbox{prior} \times \mbox{likelihood}$$
In our case, since all our parameters of interest (except LR+ and LR- which can be derived from the others) are probability parameters, the prior distribution we will use a beta prior distribution, $p\sim\beta(a,b)$, where $p$ is the probability parameter of interest, and a binomial likelihood $X\sim Bin(n,p)$, where $X$ is the number of positive results, $n$ the total number of observations and $p$ the same probability parameter as in the prior. One can show that the posterior will also be a beta distribution (since the beta is the conjugate prior for a binomial likelihood) with a closed form solution: $p|X=k\sim\beta(k+a,n-k+b)$. The closed-form solution means this is computationally extremely fast to compute. For prior, a weakly informative prior will be used, specifically the Kerman prior $\beta(1/3,1/3)$ [@kerman2011].

Specifically in our case, for a probability parameter of interest $p$, if we observe $X=k$ positive results out of $n$ total observations, then the posterior distribution will be a beta distribution with parameters $a=k+\frac{1}{3}$ and $b=n-k+\frac{1}{3}$:

$$p|n,X=k\sim\beta(k+a,n-k+b)$$

The distributional parameters $a,b$ for the posterior will be of direct interest for the health economic modelling and it are these values that will be the input to that analysis. For reporting performances of the different tests and combination of tests, we need, however, point estimates. We can easily derive these, together with 95% confidence intervals, from the estimated posterior beta distributions. As there is more than one way to do this, we need to specify how we will do this. We will:

* Derive point estimates as the posterior mode, also called maximum a posteriori estimate. This is the value that maximises the posterior distribution (i.e. the most likely value under the posterior). In our case this will be $\hat{p}=\frac{k-2/3}{n-4/3}$. This equation is not always valid (but almost always will be; we just need $0<k<n$); we will report a mode of 0 if $k=0$ and a mode of 1 if $k=n$.
...
analysisFunBayesBetaPars<-function(dat,parsList=parsList,c,r,s){
  ...
  parsListFull<-parsList
  parsList<-unique(gsub(pattern="\\.a\\.",replacement="\\.",gsub(pattern="\\.b\\.",replacement="\\.",parsList)))
  ...
  for(par in parsList){
    ...
    tmp<-unlist(strsplit(split="_",test))
    test<-tolower(unlist(strsplit(split=" ",gsub('([[:upper:]])', ' \\1', tmp[length(tmp)])))[-1])
    
    if(length(test)>1){
      datTmp<-datTmp %>% dplyr::mutate(testTmp=ifelse(rowSums(datTmp[,test])>0,1,0))
      test<-"testTmp"
    }
    
    if(test=="ssm"){test<-"reference"}
    
    if(length(tmp)>1){
      for(i in 1:(length(tmp)-1)){
        tmp2<-tolower(unlist(strsplit(split=" ",gsub('([[:upper:]])', ' \\1', tmp[1])))[-1])
        if(length(tmp2)==1){
          datTmp<-datTmp[datTmp[,tmp2]==1,]
        }else{
          datTmp<-datTmp[rowSums(datTmp[,tmp2])>0,]
        }
      }
    }
    
    if(type=="sens"){
      if(sum(datTmp$reference==1)>0){res=propEstBetaBayes(k=sum(datTmp$reference==1 & datTmp[,test]==1),n=sum(datTmp$reference==1))$parameters}else{res=rep(NA,2)}
    }else if(type=="spec"){
      if(sum(datTmp$reference==1)>0){res=propEstBetaBayes(k=sum(datTmp$reference==0 & datTmp[,test]==0),n=sum(datTmp$reference==0))$parameters}else{res=rep(NA,2)}
    }else if(type=="ppv"){
      if(sum(datTmp[,test]==1)>0){res=propEstBetaBayes(k=sum(datTmp$reference==1 & datTmp[,test]==1),n=sum(datTmp[,test]==1))$parameters}else{res=rep(NA,2)}
    }else if (type=="npv"){
      if(sum(datTmp[,test]==0)>0){res=propEstBetaBayes(k=sum(datTmp$reference==0 & datTmp[,test]==0),n=sum(datTmp[,test]==0))$parameters}else{res=rep(NA,2)} 
    }else if(type=="posProp"){
      res<-propEstBetaBayes(k=sum(datTmp[,test]==1),n=nrow(datTmp))$parameters
    }else{
      stop("invalid parameter type specification (must be one of 'sens', 'spec', 'ppv', 'npv', 'posProp').")
    }
  ...
  res<-list(pars=parsListFull,values=resList)
  return(res)
}
...
analysisFunBayesBetaPars<-function(dat,c,r,s){
  ...
  posPropCrp=propEstBetaBayes(k=sum(dat$crp==1),n=nrow(dat))
  if(sum(dat$reference==1)>0){sensCrp=propEstBetaBayes(k=sum(dat$reference==1 & dat$crp==1),n=sum(dat$reference==1))$parameters}else{sensCrp=rep(NA,2)}
  if(sum(dat$reference==1)>0){specCrp=propEstBetaBayes(k=sum(dat$reference==0 & dat$crp==0),n=sum(dat$reference==0))$parameters}else{specCrp=rep(NA,2)}
  if(sum(dat$crp==1)>0){ppvCrp=propEstBetaBayes(k=sum(dat$reference==1 & dat$crp==1),n=sum(dat$crp==1))$parameters}else{ppvCrp=rep(NA,2)}
  if(sum(dat$crp==0)>0){npvCrp=propEstBetaBayes(k=sum(dat$reference==0 & dat$crp==0),n=sum(dat$crp==0))$parameters}else{npvCrp=rep(NA,2)}
  posPropLam=propEstBetaBayes(k=sum(dat$lam==1),n=nrow(dat))
  if(sum(dat$reference==1)>0){sensLam=propEstBetaBayes(k=sum(dat$reference==1 & dat$lam==1),n=sum(dat$reference==1))$parameters}else{sensLam=rep(NA,2)}
  if(sum(dat$reference==1)>0){specLam=propEstBetaBayes(k=sum(dat$reference==0 & dat$lam==0),n=sum(dat$reference==0))$parameters}else{specLam=rep(NA,2)}
  if(sum(dat$lam==1)>0){ppvLam=propEstBetaBayes(k=sum(dat$reference==1 & dat$lam==1),n=sum(dat$lam==1))$parameters}else{ppvLam=rep(NA,2)}
  if(sum(dat$lam==0)>0){npvLam=propEstBetaBayes(k=sum(dat$reference==0 & dat$lam==0),n=sum(dat$lam==0))$parameters}else{npvLam=rep(NA,2)}
  posPropPoolxpert=propEstBetaBayes(k=sum(dat$poolxpert==1),n=nrow(dat))
  if(sum(dat$reference==1)>0){sensPoolxpert=propEstBetaBayes(k=sum(dat$reference==1 & dat$poolxpert==1),n=sum(dat$reference==1))$parameters}else{sensPoolxpert=rep(NA,2)}
  if(sum(dat$reference==1)>0){specPoolxpert=propEstBetaBayes(k=sum(dat$reference==0 & dat$poolxpert==0),n=sum(dat$reference==0))$parameters}else{specPoolxpert=rep(NA,2)}
  if(sum(dat$poolxpert==1)>0){ppvPoolxpert=propEstBetaBayes(k=sum(dat$reference==1 & dat$poolxpert==1),n=sum(dat$poolxpert==1))$parameters}else{ppvPoolxpert=rep(NA,2)}
  if(sum(dat$poolxpert==0)>0){npvPoolxpert=propEstBetaBayes(k=sum(dat$reference==0 & dat$poolxpert==0),n=sum(dat$poolxpert==0))$parameters}else{npvPoolxpert=rep(NA,2)}
  posPropCxr=propEstBetaBayes(k=sum(dat$cxr==1),n=nrow(dat))
  if(sum(dat$reference==1)>0){sensCxr=propEstBetaBayes(k=sum(dat$reference==1 & dat$cxr==1),n=sum(dat$reference==1))$parameters}else{sensCxr=rep(NA,2)}
  if(sum(dat$reference==1)>0){specCxr=propEstBetaBayes(k=sum(dat$reference==0 & dat$cxr==0),n=sum(dat$reference==0))$parameters}else{specCxr=rep(NA,2)}
  if(sum(dat$cxr==1)>0){ppvCxr=propEstBetaBayes(k=sum(dat$reference==1 & dat$cxr==1),n=sum(dat$cxr==1))$parameters}else{ppvCxr=rep(NA,2)}
  if(sum(dat$cxr==0)>0){npvCxr=propEstBetaBayes(k=sum(dat$reference==0 & dat$cxr==0),n=sum(dat$cxr==0))$parameters}else{npvCxr=rep(NA,2)}

  return(c(prevalenceTB,posPropHiv,sensHiv,specHiv,ppvHiv,npvHiv,posPropCrp,sensCrp,specCrp,ppvCrp,npvCrp,posPropLam,sensLam,specLam,ppvLam,npvLam,posPropPoolxpert,sensPoolxpert,specPoolxpert,ppvPoolxpert,npvPoolxpert,posPropCxr,sensCxr,specCxr,ppvCxr,npvCxr))
}
...
analysisFunBayes<-function(dat,c,r,s){
  ...
  posPropCrp=propEstCiBayes(k=sum(dat$crp==1),n=nrow(dat))
  sensCrp=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$crp==1),n=sum(dat$reference==1)),NA)
  specCrp=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$crp==0),n=sum(dat$reference==0)),NA)
  ppvCrp=ifelse(sum(dat$crp==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$crp==1),n=sum(dat$crp==1)),NA)
  npvCrp=ifelse(sum(dat$crp==0)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$crp==0),n=sum(dat$crp==0)),NA)
  posPropLam=propEstCiBayes(k=sum(dat$lam==1),n=nrow(dat))
  sensLam=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$lam==1),n=sum(dat$reference==1)),NA)
  specLam=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$lam==0),n=sum(dat$reference==0)),NA)
  ppvLam=ifelse(sum(dat$lam==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$lam==1),n=sum(dat$lam==1)),NA)
  npvLam=ifelse(sum(dat$lam==0)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$lam==0),n=sum(dat$lam==0)),NA)
  posPropPoolxpert=propEstCiBayes(k=sum(dat$poolxpert==1),n=nrow(dat))
  sensPoolxpert=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$poolxpert==1),n=sum(dat$reference==1)),NA)
  specPoolxpert=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$poolxpert==0),n=sum(dat$reference==0)),NA)
  ppvPoolxpert=ifelse(sum(dat$poolxpert==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$poolxpert==1),n=sum(dat$poolxpert==1)),NA)
  npvPoolxpert=ifelse(sum(dat$poolxpert==0)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$poolxpert==0),n=sum(dat$poolxpert==0)),NA)
  posPropCxr=propEstCiBayes(k=sum(dat$cad==1),n=nrow(dat))
  sensCxr=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$cxr==1),n=sum(dat$reference==1)),NA)
  specCxr=ifelse(sum(dat$reference==1)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$cxr==0),n=sum(dat$reference==0)),NA)
  ppvCxr=ifelse(sum(dat$cxr==1)>0,propEstCiBayes(k=sum(dat$reference==1 & dat$cxr==1),n=sum(dat$cxr==1)),NA)
  npvCxr=ifelse(sum(dat$cxr==0)>0,propEstCiBayes(k=sum(dat$reference==0 & dat$cxr==0),n=sum(dat$cxr==0)),NA)
...
for(i in 1:nrow(dfResPrimBayes)){
  dfResPrimBayes[i,-(1:3)]<-analysisFunBayes(dat=dfSim,c=dfResPrimBayes$country[i],r=dfResPrimBayes$region[i],s=dfResPrimBayes$setting[i])
}

dfResPrimBayesPosProp<-dfResPrimBayes %>% dplyr::select(country,region,setting,prevalenceTB,posPropHiv,posPropCrp,posPropLam,posPropPoolxpert,posPropCxr)
dfResPrimBayesSensSpec<-dfResPrimBayes %>% dplyr::select(country,region,setting,prevalenceTB,sensHiv,specHiv,sensCrp,specCrp,sensLam,specLam,sensPoolxpert,specPoolxpert,sensCxr,specCxr)
dfResPrimBayesPpvNpv<-dfResPrimBayes %>% dplyr::select(country,region,setting,prevalenceTB,ppvHiv,npvHiv,ppvCrp,npvCrp,ppvLam,npvLam,ppvPoolxpert,npvPoolxpert,ppvCxr,npvCxr)
```

### Sensitivities and specificities for each test

For every diagnostic assay, we will compute and report the sensitivity, specificity together with highest posterior density 95% confidence intervals by country and setting (@tbl-primAnalBayesSensSpec).
...
dfResPrimBayesSensSpec %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence",rep(c("Sensitivity","Specificity"),5)),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
  add_header_above(header=c(" "=3,"HIV"=2,"POC"=2,"LAM"=2,"Pooled Xpert"=2,"Digital chest X-ray + CAD"=2)) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
  pack_rows("Kenya",9,10) %>%
  pack_rows("Bangladesh",11,13) %>%
...
We will also report positive predictive and negative predictive values, also with highest posterior density 95% confidence intervals (@tbl-primAnalBayesPpvNpv).

```{r primAnalBayesPpvNpv}
#| label: tbl-primAnalBayesPpvNpv
#| tbl-cap: "Positive and negative predictive values for the evaluated tests."

dfResPrimBayesPpvNpv %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence",rep(c("PPV","NPV"),5)),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
  add_header_above(header=c(" "=3,"HIV"=2,"POC"=2,"LAM"=2,"Pooled Xpert"=2,"Digital chest X-ray + CAD"=2)) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
...
We will also compute the proportions of tests yielding positive (resp. negative) results for each test in each setting (@tbl-primAnalBayesPosProp).

```{r primAnalBayesPosProp}
#| label: tbl-primAnalBayesPosProp
#| tbl-cap: "Proportions of positive test results for the evaluated tests."

dfResPrimBayesPosProp %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence","HIV","POC","LAM","Pooled Xpert","Digital chest X-ray + CAD"),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
  pack_rows("Kenya",9,10) %>%
...
**Jan15 discussion:** pre-specified algorithms or all 405 sets of results for each setting? If the latter, how to select the best performing combination; i.e. optimisation criteria for classification models to identify the best combinations / sequences?
...
As mentioned, all of the above are probability parameters and they will directly inform the decision tree model used in the planned health economic analysis. To allow propagating the uncertainty of each of the above estimates, we will report the estimated values for $a,b$ for the posterior beta distributions and these will be the input to the health economic analysis.
...
```{r}
analysisFunBayesBetaParsAlgo<-function(dat,c,r,s,prior_a=1/3,prior_b=1/3){
  dat<-dat %>% dplyr::filter(country==c & region==r & setting==s)
  
  prevalenceTB=propEstBetaBayes(k=sum(dat$reference),n=nrow(dat),prior_a=prior_a,prior_b=prior_b)$parameters
  
  included<-rep(TRUE,nrow(dat))
  posRes<-included & (dat$cad==1)
  posPropCxr=propEstBetaBayes(k=sum(posRes),n=sum(included),prior_a=prior_a,prior_b=prior_b)$parameters
  if(sum(posRes)>0){ppvCxr=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{ppvCxr=rep(NA,2)}
  if(sum(!posRes)>0){npvCxr=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(!posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{npvCxr=rep(NA,2)}
  
  included<-(dat$cxr==1)
  posRes<-included & (dat$lam==1 | dat$crp==1 | dat$hiv==1)
  posPropLamCrpHiv=propEstBetaBayes(k=sum(posRes),n=sum(included),prior_a=prior_a,prior_b=prior_b)$parameters
  if(sum(posRes)>0){ppvLamCrpHiv=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{ppvLamCrpHiv=rep(NA,2)}
  if(sum(!posRes)>0){npvLamCrpHiv=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(!posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{npvLamCrpHiv=rep(NA,2)}
  
  included<-(dat$cxr==1 & (dat$lam==1 | dat$crpPoc==1 | dat$hiv==1))
  posRes<-included & (dat$xpert==1)
  posPropXpert=propEstBetaBayes(k=sum(posRes),n=sum(included),prior_a=prior_a,prior_b=prior_b)$parameters
  if(sum(posRes)>0){ppvXpert=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{ppvXpert=rep(NA,2)}
  if(sum(!posRes)>0){npvXpert=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(!posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{npvXpert=rep(NA,2)}
  
  included<-rep(TRUE,nrow(dat))
  posRes<-included & (dat$cxr==1 & (dat$lam==1 | dat$crp==1 | dat$hiv==1) & dat$xpert==1)
  posPropOverall=propEstBetaBayes(k=sum(posRes),n=sum(included),prior_a=prior_a,prior_b=prior_b)$parameters
  if(sum(posRes)>0){ppvOverall=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{ppvOverall=rep(NA,2)}
  if(sum(!posRes)>0){npvOverall=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(!posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{npvOverall=rep(NA,2)}
  if(sum(dat$reference==1)>0){sensOverall=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(dat$reference==1),prior_a=prior_a,prior_b=prior_b)$parameters}else{sensOverall=rep(NA,2)}
  if(sum(dat$reference==1)>0){specOverall=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(dat$reference==0),prior_a=prior_a,prior_b=prior_b)$parameters}else{specOverall=rep(NA,2)}

  return(c(prevalenceTB,posPropCxr,ppvCxr,npvCxr,posPropLamCrpHiv,ppvLamCrpHiv,npvLamCrpHiv,posPropXpert,ppvXpert,npvXpert,posPropOverall,ppvOverall,npvOverall,sensOverall,specOverall))
}
```

We only report positive proportions, PPV, NPV for each stage, but also report overall TB prevalence according to the reference test, as well as positive proportion, PPV, NPV, sensitivity and specificity for the full algorithm; see @tbl-primAnalBayesAlgo.

```{r primAnalBayesAlgo}
#| label: tbl-primAnalBayesAlgo
#| tbl-cap: "Results for an example screening algorithm. We only report the posterior beta distribution parameters for each estimated probability parameter."

dfResPrimBayesBetaAlgo<-data.frame(
  country=c(rep("Cameroon",4),rep("Nigeria",4),rep("Kenya",2),rep("Bangladesh",3),rep("Brazil",2),rep("Vietnam",3),rep("Malawi",1)),
  region=c(rep("",4+4+2+3),"Aracaju","Maceio",rep("",3+1)),
  setting=c("PHC","District","Informal settlements ACF","Children","PHC","District","Nomads","IDP / refugees","PHC","District","PHC","District","Informal settlements ACF","PHC","PHC","PHC","Informal settlements ACF","Children","PHC"),
...
for(i in 1:nrow(dfResPrimBayesBetaAlgo)){
  dfResPrimBayesBetaAlgo[i,-(1:3)]<-analysisFunBayesBetaParsAlgo(dat=dfSim,c=dfResPrimBayesBetaAlgo$country[i],r=dfResPrimBayesBetaAlgo$region[i],s=dfResPrimBayesBetaAlgo$setting[i])
}

dfResPrimBayesBetaAlgo %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting",rep(c("a","b"),15)),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).",digits=2) %>%
  kable_styling(full_width=FALSE) %>%
  ...
  add_header_above(c(" "=4,"CAD CXR"=6,"LAM + CRP + HIV | CAD CXR positive"=6,"Xpert single | LAM + CRP + HIV positive, CAD CXR positive"=6,"Overall"=10)) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
  pack_rows("Kenya",9,10) %>%
  pack_rows("Bangladesh",11,13) %>%
...
write.csv(dfResPrimBayesBetaAlgo,row.names=F,file=paste(sep="","Output/S4A_exampleAlgorithm_betaDistributionParameterEstimates_",gsub(pattern="-",replacement="",Sys.Date()),".csv"))
```
