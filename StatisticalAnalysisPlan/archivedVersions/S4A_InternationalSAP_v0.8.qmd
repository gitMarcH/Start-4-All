...
This Statistical Analysis Plan (SAP) v0.6 is based on the Start 4 All International Protocol v1.6, dated 22 May 2023.
...
dfResPrimBayesSensSpec %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence",rep(c("Sensitivity","Specificity"),6)),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
...
dfResPrimBayesPpvNpv %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence",rep(c("PPV","NPV"),6)),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
  add_header_above(header=c(" "=3,"HIV"=2,"POC CRP"=2,"RDT CRP"=2,"Fuji LAM"=2,"Pooled Xpert"=2,"Digital chest X-ray + CAD"=2)) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
...
dfResPrimBayesPosProp %>%
  dplyr::select(!country) %>%
  kable(row.names=F,col.name=c("Region","Setting","TB prevalence","HIV","POC CRP","RDT CRP","Fuji LAM","Pooled Xpert","Digital chest X-ray + CAD"),caption="Numbers in brackets indicate 95% Bayesian confidence intervals (highest posterior density intervals).") %>%
  kable_styling(full_width=FALSE) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
  pack_rows("Kenya",9,10) %>%
...
dfResPrimBetaBayes %>%
  dplyr::select(!country) %>%
  kable(row.names=FALSE,col.names=c("Region","Setting",rep(c("a","b"),31)),digits=2) %>%
  kable_styling(full_width=FALSE) %>%
...
analysisFunBayesBetaParsAlgo<-function(dat,c,r,s,prior_a=1/3,prior_b=1/3){
  ...
  posPropCad=propEstBetaBayes(k=sum(posRes),n=sum(included),prior_a=prior_a,prior_b=prior_b)$parameters
  if(sum(posRes)>0){ppvCad=propEstBetaBayes(k=sum(dat$reference==1 & posRes),n=sum(posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{ppvCad=rep(NA,2)}
  if(sum(!posRes)>0){npvCad=propEstBetaBayes(k=sum(dat$reference==0 & !posRes),n=sum(!posRes),prior_a=prior_a,prior_b=prior_b)$parameters}else{npvCad=rep(NA,2)}
...
dfResPrimBayesBetaAlgo %>%
  ...
  add_header_above(c(" "=4,"CAD CXR"=6,"LAM + CRP POC + HIV | CAD CXR positive"=6,"Xpert single | LAM + CRP POC + HIV positive, CAD CXR positive"=6,"Overall"=10)) %>%
  pack_rows("Cameroon",1,4) %>%
  pack_rows("Nigeria",5,8) %>%
  pack_rows("Kenya",9,10) %>%
  pack_rows("Bangladesh",11,13) %>%
