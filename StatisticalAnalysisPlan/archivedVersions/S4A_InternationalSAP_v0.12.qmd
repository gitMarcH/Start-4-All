...
parsList<-read.csv(header=F,"../../output/20240723/pcfIcfColnames.txt")[,1]

jagsFile<-"S4A_jagsPerformanceModel_pcficf_20240717_clean.jags"
nChains<-4
nAdapt<-3000
nBurn<-1000
nIter<-12000
...
parsVectPcvIcf<-c(parsVect1,parsVect2)
#parsVectPcvIcf<-parsVectPcvIcf[!grepl(parsVectPcvIcf,pattern="lrm|lrp")]
...
for(i in 1:nrow(dfResPrimBayes)){
  ...
  if(dfResPrimBayes$setting[i] %in% c("PHC","District")){
    res<-analysisFunBayesBetaPars(dat=dfSim,parsList=parsVectPcvIcf,c=dfResPrimBayes$country[i],r=dfResPrimBayes$region[i],s=dfResPrimBayes$setting[i],doPlot=TRUE,plotFile=paste(sep="","S4A_mcmcPars_",dfResPrimBayes$country[i],"_",dfResPrimBayes$region[i],"_",dfResPrimBayes$setting[i],"_",gsub(pattern="-",replacement="",Sys.Date()),".pdf"),nChains=nChains,nAdapt=nAdapt,nBurn=nBurn,nIter=nIter,jagsFile=jagsFile,parsVect=parsVectPcvIcf)
    ...
    values<-numeric(0)
    for(j in 1:length(parsList)){
      parTmp<-unlist(strsplit(split="\\.",parsList[j]))
    ...
    cat("ACF processing not yet implemented.\n")
    values<-rep(NA,length(parsList))
  ...
  if(i==1){
    ...
    dfRes<-rbind(dfRes,values)
  }
}

dfRes<-cbind(dfResPrimBayes,dfRes)
colnames(dfRes)<-c(colnames(dfResPrimBayes),parsList)

write.csv(dfRes,row.names=F,file=paste(sep="","mockData_AnalysisResults_PCFICF_",gsub(pattern="-",replacement="",Sys.Date()),".csv"))
save(allRes,file=paste(sep="","mockData_AnalysisResults_PCFICF_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
```
...
10. CRP --> LAM
11. LAM --> Xpert	
12. Xpert with Pooling --> Xpert	
13. CXR+CRP --> Xpert		
14. CXR+CRP --> LAM		
15. CXR+LAM --> Xpert	
16. CRP+LAM --> Xpert	
17. CXR+CRP+LAM --> Xpert
18. CXR --> CRP+LAM
...
20. CXR --> CRP --> LAM	
21. CXR --> LAM --> Xpert	
22. CXR --> Xpert with Pooling --> Xpert	
23. CRP --> Xpert with Pooling --> Xpert	
24. LAM --> Xpert with Pooling --> Xpert	
25. CXR --> CRP+LAM --> Xpert	
26. CXR+CRP --> Xpert with Pooling --> Xpert	
27. CXR+LAM --> Xpert with Pooling --> Xpert	
28. CRP+LAM --> Xpert with Pooling --> Xpert	
29. CXR+CRP+LAM --> Xpert with Pooling --> Xpert	

4-STEP ALGORITHMS

30. CXR --> CRP --> Xpert with Pooling --> Xpert
31. CXR --> LAM --> Xpert with Pooling --> Xpert
32. CXR --> CRP+LAM --> Xpert with Pooling --> Xpert
...
15. W4SS --> CRP --> LAM	
16. W4SS --> LAM --> Xpert
17. W4SS --> Xpert with Pooling --> Xpert	
18. W4SS --> CXR+CRP --> Xpert	
19. W4SS --> CXR+CRP --> LAM	
20. W4SS --> CXR+LAM --> Xpert	
21. W4SS --> CRP+LAM --> Xpert	
22. W4SS --> CXR+CRP+LAM --> Xpert
23. CXR --> CRP --> Xpert 	
24. CXR --> CRP --> LAM	
25. CXR --> LAM --> Xpert	
26. CXR --> CRP+LAM --> Xpert	
27. CXR --> Xpert with Pooling --> Xpert	
28. W4SS+CXR --> CRP --> Xpert 	
29. W4SS+CXR --> CRP --> LAM	
30. W4SS+CXR --> LAM --> Xpert	
31. W4SS+CXR --> CRP+LAM --> Xpert
32. W4SS+CXR --> Xpert with Pooling --> Xpert

4-STEP ALGORITHMS

33. W4SS --> CXR --> Xpert with Pooling --> Xpert
34. W4SS --> CRP --> Xpert with Pooling --> Xpert
35. W4SS --> LAM --> Xpert with Pooling --> Xpert
36. W4SS --> CXR+CRP --> Xpert with Pooling --> Xpert
37. W4SS --> CXR+LAM --> Xpert with Pooling --> Xpert
38. W4SS --> CRP+LAM --> Xpert with Pooling --> Xpert
39. W4SS --> CXR+CRP+LAM --> Xpert with Pooling --> Xpert
40. CXR --> CRP --> Xpert with Pooling --> Xpert
41. CXR --> LAM --> Xpert with Pooling --> Xpert
42. CXR --> CRP+LAM --> Xpert with Pooling --> Xpert
43. W4SS+CXR --> CRP --> Xpert with Pooling --> Xpert
44. W4SS+CXR --> LAM --> Xpert with Pooling --> Xpert
45. W4SS+CXR --> CRP+LAM --> Xpert with Pooling --> Xpert
