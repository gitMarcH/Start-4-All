...
This Statistical Analysis Plan (SAP) v0.10 is based on the Start 4 All International Protocol v1.6, dated 22 May 2023.
...
This means that for the prior distribution of each parameter we could assume a beta prior distribution, $p\sim\beta(a,b)$, where $p$ is the probability parameter of interest. The corresponding data likelihood will be a binomial likelihood $X\sim Bin(n,p)$, where $X$ is the number of positive results, $n$ the total number of observations and $p$ the same probability parameter as in the prior. One can show that the posterior will also be a beta distribution (since the beta is the conjugate prior for a binomial likelihood) with a closed form solution: $p|X=k\sim\beta(k+a,n-k+b)$. The closed-form solution means this would be computationally extremely fast to compute. For prior, a weakly informative prior could be used, for example the Kerman prior $\beta(1/3,1/3)$ [@kerman2011].

Specifically in this case, for a probability parameter of interest $p$, if we observe $X=k$ positive results out of $n$ total observations, then the posterior distribution would be a beta distribution with parameters $a=k+\frac{1}{3}$ and $b=n-k+\frac{1}{3}$:

$$p|n,X=k\sim\beta(k+a,n-k+b)$$

The distributional parameters $\alpha=k+a, \beta=n-k+b$ for the posterior could then be directly fed into the health economic modelling to guarantee correct propagation of errors.

However, there are several issues with this approach:

* We will need to model every single probability parameter independently.
* Missing data would have to be handled externally from the Bayesian estimation.


For these reasons, rather than using a closed form approach, we will use Markov Chain Monte Carlo (MCMC) estimation and model directly the individual test results for each test for each participant from each country and setting in the study. Only a relatively small subset of probability parameters (TB prevalence, sensitivities and specificities of individual tests) will generate all of the data, and, conditional on some relatively moderate assumptions, we can derive any and all conditional probability parameters (e.g. such as P(positive LAM | positive CAD-CXR)) directly from this small subset.

Specifically we will:

* Assume that the individual probability parameters are random variables. We will further assume that the set of sensitivity and specificity parameters for the different tests follow a non-trivial joint distribution and we will specify this in the MCMC model.

* Assume that conditional on the underlying TB status, the different tests are independent of each other.
...
4. These, or in other words, the ogits of sensitivities, specificities follow a non-trivial joint multivariate normal distribution.
...
Assuming that all dependencies between tests are captured through the above joint distribution of sensitivities and specificities and through conditioning on TB status, then all other, conditional, probability parameters follow easily from the sensitivity and specificity parameters.

For example:

* PPV and NPV follows as $p_{ppv,j}=\frac{p_{se,j}\cdot p_{TB}}{p_{se,j}\cdot p_{TB} + (1-p_{sp,j})\cdot(1-p_{TB})}$, $p_{npv,j}=\frac{p_{sp,j}\cdot(1-p_{TB})}{p_{sp,j}\cdot(1-p_{TB}) + (1-p_{se,j})\cdotp_{TB}}$.

* The probability of a positive LAM result given a positive chest x-ray is given by $P(LAM|CXR)=\frac{P(LAM,CXR)}{P(CXR)}=\frac{p_{se,lam}\cdot p_{se,cxr}\cdot p_{TB} + (1-p_{sp,lam})\cdot(1-p_{sp,cxr})\cdot(1-p_{TB})}{p_{se,cxr}\cdot p_{TB}+(1-p_{sp,cxr})\cdot(1-p_{TB})}$.

* The positive predictive value of LAM given positive chest x-ray: $p_{ppv,LAM|CXR}=P(TB|LAM,CXR)=\frac{P(TB,LAM,CRX)}{P(LAM,CXR)}=\frac{p_{TB}\cdot p_{se,lam}\cdot p_{se,cxr}}{ p_{TB}\cdot p_{se,lam}\cdot p_{se,cxr} + (1-p_{TB})\cdot (1-p_{sp,lam})\cdot(1-p_{sp,cxr})}$

* The negative predictive value of LAM given positive chest x-ray: $p_{npv,LAM|CXR}=P(\bar{TB}|\bar{LAM},CXR)=\frac{P(\bar{TB},\bar{LAM},CRX)}{P(\bar{LAM},CXR)}=\frac{(1-p_{TB})\cdot p_{sp,LAM}\cdot (1-p_{sp,CXR})}{p_{TB}\cdot (1-p_{se,lam})\cdot p_{se,cxr} + (1-p_{TB})\cdot p_{sp,lam}\cdot(1-p_{sp,cxr})}$

Other parameters follow similarly, each time assuming independence of tests conditional on TB status.

```{r primAnalBayes}
testList<-c("w4ss","crp","cxr","lam","poolxpert","xpert")
parsList<-read.csv(header=F,"../../output/20240513/pcfIcfColnames.txt")[,1]

jagsFile<-"S4A_jagsPerformanceModel_20240629.jags"
nChains<-4
nAdapt<-2500
nIter<-5000
